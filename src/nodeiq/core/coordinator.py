...
- Assemble every collector's returned section into the top-level snapshot
  envelope defined in CONTEXT.md Section 7 and docs/snapshot_schema.md
  (`timestamp`, `hostname`, `metadata`, `system`, `cpu_memory`, ...).
...
- Populate `collection_errors` by gathering whatever each collector
  reported, rather than collectors writing to it directly (see
  docs/snapshot_schema.md Section 12).
...
-----------------------------
The coordinator will call each collector's entry-point function (e.g.
`collect() -> dict`, per PROJECT_RULES.md Section 9), catch anything it
raises, and record success or failure. Collectors never call each other,
