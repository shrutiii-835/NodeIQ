...
Phase 3.2B, once individual collectors exist for it to orchestrate. It
exists now so the rest of `nodeiq.core` (runner, result, exceptions,
collector) can be built and tested against a stable package layout,
without pretending scanning itself is already working.

Future responsibilities
------------------------
When implemented, the coordinator will:

- Build exactly one `CollectorContext` (nodeiq.core.collector) for the
  scan — `scan_start_time` and `default_timeout` — shared read-only by
  every collector it calls.
- Discover and run every collector in `nodeiq.collectors`.
...
- Assemble every collector's returned `CollectorResult.data` into the
  top-level snapshot envelope defined in CONTEXT.md Section 7 and
...
- Populate `collection_errors` by gathering the `errors` field of every
  collector's returned `CollectorResult`, rather than collectors writing to
  it directly (see docs/snapshot_schema.md Section 12 and
...
(`collect(context: CollectorContext) -> CollectorResult`, per
PROJECT_RULES.md Section 9 and docs/collector_guidelines.md), catch
anything unexpected it raises, and record the returned `CollectorResult`.
Collectors never call each other, and never talk to the coordinator except
through that one return value — this is what makes it possible to add,
remove, or fix one collector without ever touching another.
...
def run_scan() -> dict:
    ...
    will do once collectors exist to orchestrate (Phase 3.2B).
    """
    raise NotImplementedError(
        "run_scan() is a placeholder — no collectors exist yet (Phase 3.2B)."
    )
